...
class PwiLogIn extends translate(LitElement, locale) {
  ...
  static get properties() {
    return {
      ...
      /**
       * set `isContvert` to true to render
      ...
      /*
       * `_processing`  flag to know if this element is siging in. It is also user to check whether we need to activate redirection stuff
       */
      _processing: {
        type: Boolean,
      },
  ...
  render() {
    return this.isConvert ? this.renderConvert() :
    this.isSignUp ? this.renderSignUp() : this.renderSignIn();
  }
  ...
  renderConvert() {
    return html `
    <div id="convert">
      <!-- <h3>${this.translate('convertYourAccount')}</h3> -->
      ${this.renderCredentialDetails()}
      <div aria-live="polite">
          ${this.renderError()}
          ${this.renderSuccess()}
          ${this.renderForScreenReader(this._processing, html`<span>${this.translate('converting')}</span>`)}
      </div>
      <div class="layout">
        <mwc-button icon="person_add" @click="${this.convertAccountHandler}" label="${this.translate('switchYourAccount')}" raised></mwc-button>
      </div>
      ${this.hideSocial ? nothing : this.renderSocial(html`<span>${this.translate('orLinkWith')}</span>`, this.convertAccountHandler)}
    </div>
  `;
  }

  renderSocial(text, handler) {
    return html `
      <div class="break "></div>
      <div>
      ...
      <div class="social  layout">
        <mwc-button @click="${handler}" class="accent-paint" data-provider="google" outlined label="google">
        ...
        <span></span>
        <mwc-button @click="${handler}" class="accent-paint" icon="facebook" data-provider="facebook" label="facebook" outlined></mwc-button>
      </div>`;
    ...
    return html `
      <div id="sign-in">
        ...
        <div aria-live="polite">
          ${this.renderError()}
          ${this.renderSuccess()}
          ${this.renderForScreenReader(this._processing, html`<span>${this.translate('signingIn')}</span>`)}
        </div>
        ...
        <div class="button-container">
          <mwc-button id="login-button" label="${this.translate('signIn')}" ?disabled="${!(this.email && this.password)}" data-provider="password" class="accent" @click="${this.loginHandler}" raised>
            ...
            <mwc-circular-progress-four-color slot="icon" aria-label="${this.translate('signInInProcess')}" density="-5" class="progress" ?closed="${!this._processing}" indeterminate></mwc-circular-progress-four-color>
          </mwc-button>
          ${
        ...
        ${this.hideSocial ? nothing : this.renderSocial(html`<span>${this.translate('signInWith')}</span>`, this.loginHandler)}
  ...
  renderSignUp() {
    return html `
      <div id="sign-up">
        <h3>${this.translate('createAnAccount')}</h3>
        ${this.renderCredentialDetails()}
        <div aria-live="polite">
            ${this.renderError()}
            ${this.renderSuccess()}
            ${this.renderForScreenReader(this._processing, html`<span>${this.translate('signingUp')}</span>`)}
        </div>
        <div class="button-container">
        ...
        ${this.hideSocial ? nothing : this.renderSocial(html`<span>${this.translate('orSignUpWith')}</span>`, this.loginHandler)}
        <div class="break "></div>
        <div class="button-container">
          <div></div>
          <span></span>
  ...
  _handleKeydown(e) {
    ...
    if (e.key === 'Enter') {
      if(this.isConvert) {
        return this.convertAccountHandler();
      }
      if (this.isSignUp) {
  ...
  convertAccountHandler(e) {
    // if (this.email && this.password) {
      this.provider = e && e.currentTarget && e.currentTarget.dataset.provider || 'password';
      this._processing = true;
      const displayName = this.displayName || this.email.replace(/@.*/, '');
      // this.provider = 'password';
      this.dispatchEvent(new CustomEvent('auth-convert', {
        detail: {
          provider: this.provider,
          email: this.email,
          ...
          successCb: () => { 
            this._processing = false;
            this.successMsg = `Success: you account is now a real account`;
